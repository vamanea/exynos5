...
/*
 * @file    csc_tiled_to_linear_y_stride_neon.s
 ...
 *
 * @param stride
 *   real width = width + stride[in]. It should be even.
 *
 */
    .arch armv7-a
    .text
    .global csc_tiled_to_linear_y_stride_neon
    .type   csc_tiled_to_linear_y_stride_neon, %function
csc_tiled_to_linear_y_stride_neon:
    .fnstart
...
LOOP_MAIN_ALIGNED_WIDTH:
    ...
    pld         [r8, #(CACHE_LINE_SIZE*PRE_LOAD_OFFSET)]
    vld1.8      {q4, q5}, [r8]!
    add         r12, r2, r4             @ temp1 = (width + stride) * i + j;
    vld1.8      {q6, q7}, [r8]!
    mul         r12, r12, r5            @ temp1 = (width + stride) * i + j;
    pld         [r8, #(CACHE_LINE_SIZE*PRE_LOAD_OFFSET)]
    vld1.8      {q8, q9}, [r8]!
    add         r12, r12, r6            @ temp1 = (width + stride) * i + j;
    vld1.8      {q10, q11}, [r8]!
...
LOOP_MAIN_REMAIN_WIDTH:
    ...
    sub         r8, r8, r14
    sub         r7, r7, r14
    add         r8, r8, #16             @ src_offset += 16;
    add         r7, r7, r2              @ dst_offset += width + stride;
    add         r7, r7, r4              @ dst_offset += width + stride;
...
REMAIN_HEIGHT_REMAIN_WIDTH_START:
    ...
    add         r12, r2, r4             @ temp1 = width * i + j;
    mul         r12, r12, r5            @ temp1 = width * i + j;
    add         r12, r12, r6
    add         r7, r0, r12             @ r7 = y_dst + temp1

    stmfd       sp!, {r0-r1,r3, r5}         @ backup registers
    mov         r0, #0
    sub         r1, r3, r9
...
LOOP_REMAIN_HEIGHT_REMAIN_WIDTH_HEIGHT1_WIDTHx:
    ...
    ldmfd       sp!, {r0-r1,r3, r5}     @ restore registers
