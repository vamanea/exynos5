...
/*
 * @file    csc_tiled_to_linear_uv_deinterleave_stride_neon.s
 ...
 *
 * @param yuv420_width
 *   width of YUV420[in]. It should be even.
 *
 * @param yuv420_height
 *   real height of YUV420[in] It should be even.
 *
 * @param stride
 *   real height = width + stride[in] It should be even.
 */

    .arch armv7-a
    .text
    .global csc_tiled_to_linear_uv_deinterleave_stride_neon
    .type   csc_tiled_to_linear_uv_deinterleave_stride_neon, %function
csc_tiled_to_linear_uv_deinterleave_stride_neon:
    .fnstart
    ...
    mov         r11, r11, asr #4
    mov         r11, r11, lsl #4
    stmfd       sp!, {r11}              @ backup tiled_width

    mov         r5, #0
LOOP_MAIN_ALIGNED_HEIGHT:
    ldr         r11, [sp]               @ r11 = tiled_width
    mul         r8, r11, r5             @ src_offset = tiled_width * i
	ldr         r11, [sp, #48]          @ r11 = stride
    mov         r6, #0
    add         r8, r2, r8              @ src_offset = uv_src + src_offset
LOOP_MAIN_ALIGNED_WIDTH:
    mov         r12, r3, asr #1         @ temp1 = (width >> 1 + sdride >> 1) * i + (j >> 1)
    add         r12, r12, r11, asr #1   @ temp1 = (width >> 1 + sdride >> 1) * i + (j >> 1)
    mul         r12, r12, r5
    ...
    pld         [r8, #(CACHE_LINE_SIZE*PRE_LOAD_OFFSET)]
    vld2.8      {q4, q5}, [r8]!
    mov         r14, r3, asr #1         @ temp2 = width / 2 + stride / 2
    add         r14, r14, r11, asr #1   @ temp2 = width / 2 + stride / 2
    vld2.8      {q6, q7}, [r8]!
...
LOOP_MAIN_REMAIN_WIDTH:
    ...
    sub         r12, r12, r14
    add         r8, r8, #16
    add         r7, r7, r3, asr #1      @ u_dst += (width >> 1) + (stride >> 1);
    add         r7, r7, r11, asr #1
    add         r12, r12, r3, asr #1    @ v_dst += (width >> 1) + (stride >> 1);
    add         r12, r12, r11, asr #1
...
LOOP_REMAIN_HEIGHT_WIDTH16:
    ...
    ldr         r11, [sp, #48]          @ r11 = stride
    mov         r12, r3, asr #1         @ temp1 = (width >> 1 + stride) * i + (j >> 1)
    add         r12, r12, r11, asr #1
    mul         r12, r12, r5
    add         r12, r12, r6, asr #1
    add         r7, r0, r12             @ r7 = u_dst + temp1
...
REMAIN_HEIGHT_REMAIN_WIDTH_START:
    ...
    ldr         r11, [sp, #48]          @ r11 = stride
    mov         r12, r3, asr #1         @ temp1 = (width >> 1 + stride / 2) * i + (j >> 1)
    add         r12, r12, r11, asr #1
    mul         r12, r12, r5
    add         r12, r12, r6, asr #1
    add         r7, r0, r12             @ r7 = u_dst + temp1
...
REMAIN_HEIGHT_END:
    ...
    ldmfd       sp!, {r11}              @ restore tiled_width
